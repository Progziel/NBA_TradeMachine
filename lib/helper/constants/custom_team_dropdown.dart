...
class CustomTeamDropDown extends StatefulWidget {
  final List<TeamsModel> teamName;
  final TeamsModel? value;
  final String dropDownText;
  final Function(dynamic) onChanged;
  final double? width;
  const CustomTeamDropDown(
      {super.key,
...
class _CustomTeamDropDownState extends State<CustomTeamDropDown> {
  ...
  Widget build(BuildContext context) {
    return DropdownButtonHideUnderline(
      child: DropdownButton2<TeamsModel>(
        isExpanded: true,
        hint: Padding(
        ...
        items: widget.teamName
            .map((TeamsModel item) => DropdownMenuItem<TeamsModel>(
          value: item,
          child: Padding(
